r.transition <- function(N, mu, sigma){ # choose any f transition dist
  rnorm(N, mu, sigma)
}
#
d.emission <- function(y, mu, sigma){ # choose g(y|x) emission dist
  dnorm(y, mu, sigma)
}

r.initial_distr <- function(N, mu, sigma){ # choose any mu initial dist
  rnorm(N, mu, sigma)
}
